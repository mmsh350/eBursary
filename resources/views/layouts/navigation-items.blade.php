...
@if (auth()->user()?->hasRole('admin'))
    ...
    <li>
        <a href="{{ route('vendors.index') }}"
            class="{{ request()->routeIs('vendors.index') ? 'bg-blue-800 text-white' : 'text-blue-100 hover:text-white hover:bg-blue-800' }} group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold transition-all duration-200">
            <svg class="h-6 w-6 shrink-0 {{ request()->routeIs('vendors.index') ? 'text-white' : 'text-blue-200 group-hover:text-white' }}"
                fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round"
